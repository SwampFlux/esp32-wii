...
static void process_command_complete_event(uint8_t len, uint8_t *data)
{
  ...
  else if (data[1] == 0x09 && data[2] == 0x10)
  { // read_bd_addr
    // data[0] Num_HCI_Command_Packets
    if (data[3] == 0x00)
    { // OK
  ...
  else if (data[1] == 0x13 && data[2] == 0x0C)
  { // write_local_name
    // data[0] Num_HCI_Command_Packets
    if (data[3] == 0x00)
    { // OK
  ...
  else if (data[1] == 0x24 && data[2] == 0x0C)
  ...
  else if (data[1] == 0x1A && data[2] == 0x0C)
  { // write_scan_enable
    // data[0] Num_HCI_Command_Packets
    if (data[3] == 0x00)
    { // OK
  ...
  else if (data[1] == 0x02 && data[2] == 0x04)
  { // inquiry_cancel
    // data[0] Num_HCI_Command_Packets
    if (data[3] == 0x00)
    { // OK
  ...
  case 3:
    ...
    if (data[1] == 0x21)
    {
      if (memcmp(data + 5, (const uint8_t[]){0x00, 0xFA}, 2) == 0)
      {
        ...
        if (memcmp(data + 7, (const uint8_t[]){0x00, 0x00, 0xA4, 0x20, 0x04, 0x02}, 6) == 0)
        {                                                                  // Wii Balance Board
...
float balance_interpolate(uint8_t pos, uint16_t *values, uint16_t *cal)
{
  ...
  if (values[pos] < cal[pos])
  { // 0kg
    weight = 0;
  }
  else if (values[pos] < cal[pos + 4])
  { // 17kg
    weight = 17 * (float)(values[pos] - cal[pos]) / (float)(cal[pos + 4] - cal[pos]);
  }
  else /* if (values[pos] > cal[pos+5])*/
  {    // 34kg
